...
resource "aws_instance" "this" {
  ...
  ami                  = try(data.aws_ami.ami[0].id, data.aws_ami.backup_ami[0].id, var.instance_spec.ami)
  ...
  credit_specification {
    cpu_credits = contains(["t2", "t3", "t3a"], substr(var.instance_type, 0, 2)) ? var.cpu_credits : null
  }
